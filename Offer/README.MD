...
后序遍历：从下往上遍历，如果子树是平衡二叉树，则返回子树的高度
...
public class Solution {
    ...
    private int getDepth(TreeNode root,int level,boolean[] res){
        if(root == null)
            return level;
        int L = getDepth(root.left,level+1,res);
        if(!res[0])
            return level;
        int R = getDepth(root.right,level+1,res);
        if(!res[0])
            return level;
        if(Math.abs(L-R)>1)
            res[0]=false;
...
根据中序遍历的规则
1、如果一个结点有右子树，那它下一个结点就是右子树的最左结点。
2、如果没有右子树，则需要判断它有没有父结点并且是不是父结点的左子结点，是的话，就找到了。
不是则要不断地向上找，直到某一个结点是其父结点的左子结点，返回这个父结点即为所求。
...
递归判断：
1、左子树的左子树和右子树的右子树是否相同
2、左子树的右子树和右子树的左子树是否相同
/*
public class TreeNode {
    int val = 0;
    TreeNode left = null;
...
public class Solution {
    ...
    boolean subRoot(TreeNode left,TreeNode right){
        if(left==null)
            return right==null;
        if(right==null)
            return false;
        if(left.val!=right.val)
            return false;
        return subRoot(left.left,right.right)&&subRoot(left.right,right.left);
    }
}

非递归：
与递归的思路相同，成对的入栈、出栈以及判断
public class Solution {
    boolean isSymmetrical(TreeNode pRoot){
        if(pRoot==null)
            return true;
        ...
        while(!stack.isEmpty()){
            ...
            if(left.val!=right.val)
                return false;
            stack.push(left.left); //成对放入，左的左、右的右
            stack.push(right.right);
            stack.push(left.right); //成对放入，左的右、右的左
...
使用2个栈，stackA用来从左到右遍历，stackB用来从右到左的遍历：
import java.util.*;
public class Solution {
...
* 序列化二叉树
```java
原理还是根据二叉树的遍历过程，用 ！ 来分隔每个节点的值（方便反序列化），
如果是空节点，就使用 # 来替代。
